...
for (row_kris in 1:nrow(kris_key)) {
  for (row in 1:nrow(k)) {
    for (col in 1:ncol(k)) {
      if (grepl(tolower(as.String(k[row, col])), tolower(as.String(kris_key[row_kris,])), fixed=TRUE) | grepl(tolower(as.String(kris_key[row_kris,])), tolower(as.String(k[row, col])), fixed=TRUE)) {
...
```{r}
PCC_key <- read_excel("../data/PCC_keywords.xlsx", header = F)
