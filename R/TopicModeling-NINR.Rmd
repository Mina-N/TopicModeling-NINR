...
ninr19 = read_excel("./../data/NINR-2019-all-apps-with-RCDC.xlsx", guess_max = 1000000)
ninr18 = read_excel("./../data/NINR-2018-all-apps-with-RCDC.xlsx", guess_max = 1000000)
ninr17 = read_excel("./../data/NINR-2017-all-apps-with-RCDC.xlsx", guess_max = 1000000)
ninr16 = read_excel("./../data/NINR-2016-all-apps-with-RCDC.xlsx", guess_max = 1000000)
ninr15 = read_excel("./../data/NINR-2015-all-apps-with-RCDC.xlsx", guess_max = 1000000)

ninr = rbind(ninr19, ninr18, ninr17, ninr16, ninr15)
...
```{r}
 ...
 pp
 write_csv(pp, "LDA_Opt_Param_NINR_15_19.csv")
```

Let's load the optimal parameters.
```{r}
pp = read_csv("LDA_Opt_Param_NINR_15_19.csv")
pp = data.frame(pp)
...
j = j$topic.order
j = as.integer(j)
k = lda_model$get_top_words(n = 20, topic_number = 1:topics, lambda = 0.25)
k = k[, j]
#k = data.frame(k)
#colnames(k) = paste0("T", 1:topics)
#k
...
p = cbind(FY=ninr$FY, p)

n = names(p)
n1 = names(select(p, FY:Topic2))
n2 = sort(n[!(n %in% n1)])

dtm2 = as.matrix(dtm)
dtm3 = data.frame(dtm2)
dtm3$Project = row.names(dtm2)
dtm4 = inner_join(select(p, Project, Topic1, PO.Name, FY), dtm3)

set.seed(123)


for (year in 2015:2019) {
  dtm_sub = filter(dtm4, FY == UQ(year))
  print(year)
  print(nrow(dtm_sub))
  ts = Rtsne(as.matrix(select(dtm_sub, -Project, -Topic1, -PO.Name)), check_duplicates=FALSE, pca=TRUE, perplexity=30, theta=0.5, dims=2)
  print('got here')
  dts = as.data.frame(ts$Y)
  dts2 = cbind(select(dtm_sub, Project, Topic1, PO.Name), dts)
  dts2 = rename(dts2, PO.Name = PO.Name)

 print(
    ggplot(dts2, aes(x = V1, y = V2, col = Topic1))
  + geom_point()
  + ggtitle(paste("Most Likely Topic of Each Grant in Term Frequency Space ", toString(year)))
  + theme_minimal()
 ...
 print(
  ...
  + geom_point()
  + ggtitle(paste("PO of Each Grant in Term Frequency Space ", toString(year)))
  + theme_minimal()
  + theme(legend.position = "bottom",
          plot.title = element_text(hjust = 0.5),
          axis.text.x = element_text(angle = 0, vjust = 0.4))
 ...
 print(
    ggplot(dts2, aes(x = V1, y = V2, col = Topic1)) 
  + geom_point()
  + ggtitle(paste("Most Likely Topic of Each Grant in Topic Space ", toString(year)))
  + theme_minimal()
  + theme(legend.position = "bottom",
          plot.title = element_text(hjust = 0.5),
          axis.text.x = element_text(angle = 0, vjust = 0.4))
 ...
 print(
    ggplot(dts2, aes(x = V1, y = V2, col = PO.Name)) 
  + geom_point()
  + ggtitle(paste("PO of Each Grant in Topic Space ", toString(year)))
  + theme_minimal()
  + theme(legend.position = "bottom",
          plot.title = element_text(hjust = 0.5),
          axis.text.x = element_text(angle = 0, vjust = 0.4)) 
...
print(
  ...
  + ylab("Frequency")
  + xlab("PO")
  + ggtitle(paste("Most Likely Topics of POs (Frequency in Portfolio) ", toString(year)))
...
print(
  ...
  + ylab("Percentage")
  + xlab("PO")
  + ggtitle(paste("Most Likely Topics of POs (Percentage of Portfolio) ", toString(year)))
  + scale_y_continuous(labels = scales::percent)
  + theme_minimal()
  + theme(legend.position = "right",
          plot.title = element_text(hjust = 0.5),
...
for (year in 2015:2019) {
  ...
  + ylab("Percentage")
  + xlab("Topic")
  + ggtitle(paste("Distribution of Topic Percentages", toString(year)))
  + scale_y_continuous(labels = scales::percent)
  + theme_minimal()
  ...
  + ylab("Percentage")
  + xlab("PO")
  + ggtitle(paste("Distribution of Topic Percentages by PO", toString(year)))
  + scale_y_continuous(labels = scales::percent)
  + theme_minimal()
  + theme(legend.position = "right",
          plot.title = element_text(hjust = 0.5),
...
for (year in 2015:2019) {
    ...
    for(topic in sort(unique(p1$Topic))){
      ...
      + ggtitle(paste("Distribution of Topic", topic, "by PO ", year))
      + theme_minimal()
      + theme(legend.position = "right",
                  plot.title = element_text(hjust = 0.5),
...
write_csv(p2, "NINR-2015-2019-constant-topics.csv")
```
...
## Histogram of topic distributions
```{r}
for (i in 1:13) {
  topic = paste0("T", i)
  plot = (
  p2 %>%
    ggplot(aes_string(x=topic)) +
    geom_histogram() +
    scale_x_continuous(breaks = round(seq(0, 100, by = 5), 1)) +
    scale_y_continuous(breaks = round(seq(0, 4300, by = 100), 1))  +
    ylim(c(0, 4300))
  )
  print(plot)
}
...
for (i in topic_num) {
  for (j in 2015:2019) {
      ...
      plot = (
        p1 %>%
        ggplot(aes_string(x=topic)) +
        ggtitle(paste("Topic", i, "Percentages in Grants in", j)) + 
        geom_histogram() +
        scale_x_continuous(breaks = round(seq(0, 100, by = 5), 1)) +
        scale_y_continuous(breaks = round(seq(0, 4300, by = 100), 1))  +
        ylim(c(0, 4300))
