...
ninr_1 = read_excel("./../data/NINR_Prevention.xlsx", guess_max = 1000000)
ninr_2 = read_excel("./../data/NICHD_Prevention.xlsx", guess_max = 1000000)
...
opt_param(ninr_dtm, "LDA_Opt_Param_NINR_NICHD_Translational.csv")

```

Let's load the optimal parameters.
```{r}
pp_ninr = read_csv("LDA_Opt_Param_NINR_NICHD_Prevention.csv")
...
k = lda_model$get_top_words(n = 20, topic_number = 1:topics, lambda = 0)
k = k[, j]
...
write_csv(p2, "NINR-NICHD-prevention-topics.csv")
```
...
nia_grants <- p2[substr(p2$Project, 1, 2) == "HD",]
...
pie(c(pie_ninr_percentage, pie_nia_percentage), c(paste0("NINR (", pie_ninr_percentage, "%)"), paste0("NICHD (", pie_nia_percentage, "%)")))
