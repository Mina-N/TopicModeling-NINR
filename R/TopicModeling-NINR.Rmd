...
new_NIA19 = read_excel("./../data/awarded-new-apps-NIA-2019.xlsx", guess_max = 1000000)
competing_NIA19 = read_excel("./../data/competing-awarded-grants-NIA-2019.xlsx", guess_max = 1000000)
noncompeting_NIA19 = read_excel("./../data/non-competing-awarded-grants-NIA-2019.xlsx", guess_max = 1000000)
noncompeting_cont_NIA19 = read_excel("./../data/non-competing-continuations-NIA-2019.xlsx", guess_max = 1000000)

nia_1 <- rbind(noncompeting_NIA19, competing_NIA19) # does not have PCC field
nia_2 <- rbind(new_NIA19, noncompeting_cont_NIA19)
drops <- c("PCC")
nia_2 <- nia_2[ , !(names(nia_2) %in% drops)]
nia <- rbind(nia_1, nia_2)
...
## Rename Columns
```{r}
names(nia)[names(nia) == 'PI Name'] <- 'PI.Name'
names(nia)[names(nia) == 'PO Name'] <- 'PO.Name'
names(nia)[names(nia) == 'RFA/PA Number'] <- 'RFA.PA.Number'
names(nia)[names(nia) == 'Stat Desc'] <- 'Stat.Desc'
names(nia)[names(nia) == 'Abstract Text (only)'] <- 'Abstract'
```
...
nia <- nia[nia$Abstract != "-",]
nia$Text <- nia[[text[1]]]

if(length(text) > 1){
  for(i in 2:length(text)){
    nia$Text <- paste(nia$Text, nia[[text[i]]])
  }
}

nia = select(nia, -c("Title", "Abstract", "SA Text"))
nia = mutate(nia, Text = ifelse(is.na(Text), "", Text))
nia = filter(nia, Text != "")
...
nia$Text = str_replace_all(nia$Text, "\n", " ")
nia$Text = str_replace_all(nia$Text, "\r", " ")
nia$Text = str_replace_all(nia$Text, "'", "")
nia$Text = str_replace_all(nia$Text, "-", " ")

# Replace non-alpha numeric characters with a space
nia$Text = str_replace_all(nia$Text, "[^abcdefghijklmnopqrstuvwxyzABCDEFHIJKLMNOPQRSTUVWXZ0123456789 ]", " ")

# Put everything in lowercase
nia$Text = tolower(nia$Text)

# Remove a few select words and extra spaces
nia$Text = str_replace_all(nia$Text, "abstract", " ")
nia$Text = str_replace_all(nia$Text, "project", " ")
nia$Text = str_replace_all(nia$Text, "proposal", " ")
nia$Text = str_replace_all(nia$Text, "summary", " ")
nia$Text = str_replace_all(nia$Text, "narrative", " ")
nia$Text = str_replace_all(nia$Text, "background", " ")
nia$Text = str_replace_all(nia$Text, "supplement", " ")
nia$Text = str_replace_all(nia$Text, "significance", " ")
nia$Text = str_replace_all(nia$Text, "overall", " ")
nia$Text = str_replace_all(nia$Text, "title", " ")
nia$Text = str_replace_all(nia$Text, "specific aims", " ")
nia$Text = str_replace_all(nia$Text, "goal", " ")
nia$Text = str_replace_all(nia$Text, "research plan", " ")
nia$Text = str_replace_all(nia$Text, "overview", " ")
nia$Text = str_replace_all(nia$Text, "\\s+", " ")
nia$Text = str_replace_all(nia$Text, "description provided by applicant", " ")

nia$Text = str_trim(nia$Text, side = "both")

```

### Unique Text
```{r}
nia <- nia[!duplicated(nia[ , c("Text")]),]
nrow(nia)
```
...
```{r}
 ...
 pp
 write_csv(pp, "LDA_Opt_Param_NIA.csv")
```

Let's load the optimal parameters.
```{r}
pp = read_csv("LDA_Opt_Param_NIA.csv")
pp = data.frame(pp)
str(pp)
```
...
```{r}
acronym_lookup <- data.frame("Acronym" = c("pa", "smm", "hf", "plwh", "t2dm", "msm", "fsw", "hana", "acp", "cds", "cqa", "cci", "comt",
                                           "bpsd", "pwd", "fc", "eolpc", "pcrc", "cmc", "4_bpsd", "pes", "mcc", "hhc", "dts", "cri",
                                           "vte", "pru", "cbpr", "cbt", "trkb", "lbp", "trkb_t1", "oa", "eol"),  
                            "Derivation" = c("physical activity", "severe maternal morbidity ", "heart failure", "persons living with HIV",
                                             "type 2 diabetes mellitus", "men who have sex with men", "female sex worker", "HIV-associated non-AIDS conditions",
                                             "advance care planning", "clinical decision support", "communication quality analysis", "chronic critical illness",
                                             "catechol-O-methyltransferase", "behavioral and psychological symptoms of dementia", "persons with dementia", "fecal calprotectin",
                                             "end of life and palliative care", "Palliative Care Research Consortium", "children with medical complexity",
                                             "behavioral and psychological symptoms of dementia", "patient engagement specialist", "multiple chronic conditions", 
                                             "home healthcare", "disinfection tracking system", "cardiorespiratory instability", "Venous thromboembolism", "pruritis",
                                             "community-based participatory research", "cognitive behavioral therapy", "tropomyosin-related receptor kinase type B", 
                                             "lower back pain", "truncated isoform of Trkb", "osteoarthritis", "end of life"))
...
p = inner_join(select(nia, Project:PO.Name), d)
...
write_csv(p2, "awarded-grants-NIA-2019-topics.csv")
```
